...
public class Settings {
    ...
    public void printParams () {
        params.forEach((k,v)-> window.infoOut(k+"\t"+v,GUI.P_LOW));
    }
    ...
    public String getCommand(int cmdnum){
            ...
            if (updateSettings()){
                switch (cmdnum){
                    case 1:     //ADC command
                        ...
                        window.infoOut(command,GUI.P_HIGH);
                        break;
                    case 2:     //Gain command
                    ...
                    case 3:     //Experiment command;
                        ...
                        break;
                    default:
                        window.dbgOut("Could not get command information, invalid command number");
                        break;
                }
            }
            return command;
        ...
        {
            window.dbgOut("Get command failed. (" + e.toString() + ")");
            return null;
        }
    }
    ...
    public String getExperimentCmd(String selectedExperiment){
        ...
        try
        {
            ...
            window.infoOut("Experiment Command: "+experimentCmd,1);
            return experimentCmd;
        }
        catch (Exception e)
        {
            window.dbgOut("Get experiment command failed. (" + e.toString() + ")");
            return "";
        }
    }
    ...
    public Range getRangeFromParams(){
        ...
        try
        {
            switch (selectedExperiment){
                ...
                case "Differential Pulse Voltammetry":
                    ...
                    break;
                default:
                    window.dbgOut("Could not obtain range from experiment parameters, experiment not found");
                    break;
            }
            window.infoOut("Range: Lower - "+ lower +" Upper - "+ upper,0);
        ...
        {
            window.dbgOut("Could not obtain range from experiment parameters, exception occurred: (" + e.toString() + ")");
    ...
    private String getInpBufferCmd(){
        ...
        if (setting.equals("True")){
            ...
            //TODO: ADDEXCEPTION
            window.dbgOut("no input buffer command");
        }
        return cmd;
    }
    private String getSampRateCmd(){
        String setting = params.get("Sample Rate");
        //window.dbgOut("samp rate:" + setting);
        String cmd = null;
        ...
        switch (setting){
            ...
            default:
                //TODO: ADDEXCEPTION
                window.dbgOut("invalid sample rate command");
                break;
        }
        return cmd;
    }
    private String get2ElecCmd() {
        ...
        if (setting.equals("True")) {
            ...
            //TODO: ADDEXCEPTION
            window.dbgOut("invalid 2 electrode command");
        }
        return cmd;
    }
    private String getPGASetCmd(){
        ...
        switch (setting){
            ...
            case "64X":
                ...
                break;
            default:
                window.dbgOut("invalid PGA command");
                break;
        }
        return cmd;
    }
    private String getGainCmd() {
        ...
        switch (setting) {
            ...
            case "100 MΩ (15 nA FS)":
                ...
                break;
            default:
                window.dbgOut("invalid Gain command");
                break;
        }
        return cmd;
    }
    private String getIntCmd(String paramKey, int max, int min){
        ...
        String cmd = "";
        if (setting.isEmpty())
            window.infoOut(String.format("Value cannot be empty: %s",paramKey),GUI.P_USER);
        else if (setval > max)
            window.infoOut(String.format("Value exceeds maximum: %s (Max = %d)",paramKey,max),GUI.P_USER);
        else if (setval < min)
            window.infoOut(String.format("Value is below minimum: %s (Min = %d)",paramKey,min),GUI.P_USER);
    ...
    public boolean updateSettings(){
        try
        {
            window.setParamsFromFields();
            return true;
        }
        catch (Exception e)
        {
            window.dbgOut("Update settings failed. (" + e.toString() + ")");
            return false;
        }
